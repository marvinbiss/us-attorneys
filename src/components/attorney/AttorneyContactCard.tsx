...
export function AttorneyContactCard({ attorney }: AttorneyContactCardProps) {
  const hasPhone = isValidPhone(attorney.phone)
        ...
        <div className="flex items-center gap-3 mb-5 flex-wrap">
          ...
          {attorney.average_rating > 0 && (
            <div className="flex items-center gap-1 text-xs text-slate-500">
              ...
              <span className="font-semibold text-gray-900">{attorney.average_rating.toFixed(1)}</span>
            </div>
          )}
        </div>
        ...
        <div className="space-y-3">
          ...
          {hasPhone && (
            <button
              type="button"
              onClick={() => {
                trackEvent('phone_reveal', { attorney_slug: attorney.slug })
                trackEvent('phone_click', { attorney_slug: attorney.slug })
              ...
              aria-label={`Call ${formatUSPhone(attorney.phone!)}`}
            >
              <Phone className="w-4 h-4" />
              {formatUSPhone(attorney.phone!)}
            </button>
          )}
          ...
          <button
            type="button"
            onClick={() => {
              trackEvent('attorney_quote_click', { attorney_slug: attorney.slug })
          ...
          {hasEmail && (
            <a
              ...
              onClick={() => {
                trackEvent('attorney_email_click', { attorney_slug: attorney.slug })
              }}
