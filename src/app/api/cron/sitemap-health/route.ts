...
export async function GET(request: Request) {
  ...
  if (!indexValidation.valid) {
    logger.error('[sitemap-health] CRITICAL: sitemap index URL blocked by SSRF filter', null, {
      action: 'ssrf-check',
      component: 'cron',
    })
    return NextResponse.json(
      { error: 'Invalid sitemap index URL', reason: indexValidation.reason },
      { status: 500 }
    )
  }
  ...
  if (!indexRes.ok) {
    logger.error(
      '[sitemap-health] CRITICAL: sitemap index returned non-200',
      new Error(`HTTP ${indexRes.status}`)
    )
    await sendAlert({
      level: 'critical',
  ...
  // Extract <loc> URLs using string splitting instead of regex to avoid ReDoS risk
  const rawUrls: string[] = []
  const locParts = indexXml.split('<loc>')
  for (let i = 1; i < locParts.length; i++) {
    const endIdx = locParts[i].indexOf('</loc>')
    if (endIdx !== -1) {
      const url = locParts[i].substring(0, endIdx).trim()
      if (url.length > 0) {
        rawUrls.push(url)
      }
    }
  ...
  if (sitemapUrls.length < rawUrls.length) {
    console.warn(
      `[sitemap-health] SSRF filter blocked ${rawUrls.length - sitemapUrls.length} URLs out of ${rawUrls.length}`
    )
  }
    ...
    const batchResults = await Promise.all(
      batch.map(async (url) => {
        try {
          ...
          const urlCount = text.split('<url>').length - 1
  ...
  if (!allOk) {
    logger.error(
      '[sitemap-health] Sitemaps failed',
      new Error(`${failures.length} sitemaps failed`),
      { failures }
    )
    await sendAlert({
