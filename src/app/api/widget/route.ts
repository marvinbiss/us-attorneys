...
/**
 * Serves a lightweight JavaScript widget that attorneys can embed on their websites.
 * Creates a styled badge linking back to US Attorneys.
 *
 * Query params:
...
export async function GET(request: Request) {
  ...
  if(ds){h+='<div style="font-size:13px;color:#64748b;margin-bottom:8px">'+ds.replace(/</g,'&lt;')+(dv?' in '+dv.replace(/</g,'&lt;'):'')+'</div>';}
  ...
  h+='View profile <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14M12 5l7 7-7 7"/></svg></a>';
  h+='<div style="margin-top:10px;font-size:10px;color:#94a3b8">Professional listed on <a href="${escapedSiteUrl}" target="_blank" rel="noopener" style="color:#2563eb;text-decoration:none">${escapedSiteName}</a></div>';
  h+='</div>';
  c.innerHTML=h;
})();`
