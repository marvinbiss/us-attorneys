...
export const GET = createApiHandler(
  async ({ request, user }) => {
    ...
    let query = adminClient
      ...
      .select(
        'id, case_id, case_type, file_name, file_size, mime_type, storage_path, shared_with_attorney, created_at, updated_at'
      )
  ...
  async ({ request, user }) => {
    ...
    const { data: doc, error: insertError } = await withTimeout(
      adminClient
        ...
        .select(
          'id, case_id, case_type, file_name, file_size, mime_type, storage_path, shared_with_attorney, created_at'
        )
        .single()
    )
...
export async function DELETE(request: Request) {
  ...
  if (!documentId) {
    return NextResponse.json(
      { success: false, error: { code: 'VALIDATION_ERROR', message: 'Document ID required' } },
      { status: 400 }
    )
  }
  ...
  const { error: deleteError } = await withTimeout(
    adminClient.from('client_documents').delete().eq('id', documentId)
  )

  if (deleteError) {
    logger.error('Client documents: delete error', deleteError)
    return NextResponse.json(
      { success: false, error: { code: 'SERVER_ERROR', message: 'Failed to delete document' } },
      { status: 500 }
    )
  }
