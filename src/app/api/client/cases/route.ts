...
export const GET = createApiHandler(
  async ({ request, user }) => {
    ...
    // 1. Fetch leads (quote_requests) for this client
    try {
      ...
      if (leads && leads.length > 0) {
        ...
        const attorneyIds = Array.from(
          new Set((assignments || []).map((a) => a.attorney_id).filter(Boolean))
        )
        let attorneyMap: Record<
          string,
          { name: string; slug: string; specialty_name: string | null }
        > = {}
          ...
          if (attorneys) {
            ...
            if (specialtyIds.length > 0) {
              const { data: specialties } = await withTimeout(
                adminClient.from('specialties').select('id, name').in('id', specialtyIds)
              )
              if (specialties) {
            ...
            attorneyMap = Object.fromEntries(
              attorneys.map((a) => [
                a.id,
                {
                  name: a.name,
                  slug: a.slug,
                  specialty_name: a.primary_specialty_id
                    ? specialtyMap[a.primary_specialty_id] || null
                    : null,
                },
              ])
            )
        ...
        const assignmentsByLead: Record<
          string,
          { lead_id: string; attorney_id: string; status: string; assigned_at: string }
        > = {}
        for (const assignment of assignments || []) {
          ...
          if (
            ...
            (assignment.status === 'viewed' && existing.status === 'pending')
          ) {
    ...
    try {
      const { data: bookings, error: bookingsError } = await withTimeout(
        adminClient
          .from('bookings')
          .select(
            'id, attorney_id, specialty_id, scheduled_at, status, notes, created_at, duration_minutes'
          )
          .eq('client_id', user?.id ?? '')
          .order('created_at', { ascending: false })
      )
      ...
      if (bookings && bookings.length > 0) {
        const bookingAttorneyIds = Array.from(
          new Set(bookings.map((b) => b.attorney_id).filter(Boolean))
        )
        let bookingAttorneyMap: Record<string, { name: string; slug: string }> = {}

        if (bookingAttorneyIds.length > 0) {
          const { data: attorneys } = await withTimeout(
            adminClient.from('attorneys').select('id, name, slug').in('id', bookingAttorneyIds)
          )
          if (attorneys) {
            bookingAttorneyMap = Object.fromEntries(
              attorneys.map((a) => [a.id, { name: a.name, slug: a.slug }])
            )
          }
        }
        ...
        for (const booking of bookings) {
          ...
          cases.push({
            ...
            attorney_slug: attorney?.slug || null,
            practice_area: booking.specialty_id
              ? specMap[booking.specialty_id] || 'Consultation'
              : 'Video Consultation',
            status,
            status_label: STATUS_LABELS[status],
