 ...
 * GET: Fetch client's consultation requests (quote_requests) with status derived from lead_events
...
export const GET = createApiHandler(
  async ({ request, user }) => {
    ...
    const stats = {
      ...
      completed: enrichedLeads.filter(
        (l) => l.derived_status === 'completed' || l.derived_status === 'accepted'
      ).length,
    }
