...
export async function GET(request: Request) {
  try {
    ...
    const result = requestsQuerySchema.safeParse(queryParams)
    if (!result.success) {
    ...
    if (!provider) {
      return NextResponse.json({
        requests: [],
        stats: { total: 0, new: 0, quotes_sent: 0, accepted: 0, declined: 0 },
      })
    }
    ...
    if (allRequestsError) {
      ...
      return NextResponse.json({ error: 'Error retrieving claims' }, { status: 500 })
    }
