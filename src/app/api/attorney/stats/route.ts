...
export const GET = createApiHandler(
  async ({ request, user }) => {
    ...
    const [{ data: profile }, { data: provider }] = await withTimeout(
      Promise.all([
        supabase
          .from('profiles')
        ...
        supabase
          .from('attorneys')
          .select(
            'id, stable_id, slug, address_city, address_zip, is_verified, name, description, phone, email, specialty:specialties!primary_specialty_id(name, slug)'
          )
          .eq('user_id', user?.id ?? '')
          .single(),
      ])
    ...
    const [
      ...
      Promise.all([
        ...
        // Conversations (for unread messages)
        supabase.from('conversations').select('id').eq('attorney_id', attorneyId),
    ...
    // -----------------------------------------------------------------------
    // Top services (from quote_requests linked via lead_assignments)
    // -----------------------------------------------------------------------
    ...
    const recentLeadIds = assignments
      .slice(0, 5)
      .map((a) => a.lead_id)
      .filter(Boolean)
