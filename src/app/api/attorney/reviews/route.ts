  ...
  async ({ user }) => {
    ...
    const { data: reviews, error: reviewsError } = await withTimeout(
      supabase
        .from('reviews')
        .select(
          'id, attorney_id, rating, comment, attorney_response, attorney_responded_at, client_name, booking_id, created_at, updated_at'
        )
    ...
    const formattedReviews =
      reviews?.map((r) => ({
        ...
        attorney_responded_at: r.attorney_responded_at,
...
export const POST = createApiHandler(
  async ({ request, user }) => {
    ...
    const { data: review } = await withTimeout(
      supabase.from('reviews').select('attorney_id, attorney_response').eq('id', review_id).single()
    )
    ...
    const { error: updateError } = await supabase
      .from('reviews')
      .update({ attorney_response: response, attorney_responded_at: new Date().toISOString() })
      .eq('id', review_id)
