...
import { getSpecialtyBySlug, getAttorneyCountByServiceAndLocation } from '@/lib/supabase'
import { getBreadcrumbSchema, getSpeakableSchema } from '@/lib/seo/jsonld'
import Breadcrumb from '@/components/Breadcrumb'
import CrossIntentLinks from '@/components/seo/CrossIntentLinks'
import { getServiceImage } from '@/lib/data/images'
import { practiceAreas as staticPracticeAreas, cities, getStateByCode } from '@/lib/data/usa'
...
const topCounties: CountyInfo[] = [
  {
    ...
    citySlugs: ['los-angeles', 'long-beach', 'santa-clarita', 'glendale', 'lancaster'],
  },
  {
    slug: 'cook-county-il',
    name: 'Cook County',
    ...
    citySlugs: ['chicago', 'aurora', 'joliet', 'naperville', 'elgin'],
  },
  {
  ...
  },
  {
    ...
    citySlugs: ['san-diego', 'chula-vista'],
  },
  {
    ...
    stateName: 'California',
    seat: 'Santa Ana',
    population: '3,167,809',
    citySlugs: ['anaheim', 'santa-ana', 'irvine'],
  },
  {
  ...
  },
  {
    ...
    citySlugs: ['dallas', 'irving', 'garland', 'grand-prairie'],
  },
  {
  ...
  },
  {
    ...
    citySlugs: ['san-bernardino', 'fontana'],
  },
  {
    slug: 'king-county-wa',
    name: 'King County',
    ...
    citySlugs: ['seattle', 'bellevue', 'kent', 'renton'],
  },
  {
    ...
    citySlugs: ['las-vegas', 'henderson', 'north-las-vegas'],
  },
  {
    ...
    citySlugs: ['fort-worth', 'arlington'],
  },
  {
  ...
  },
  {
    ...
    citySlugs: ['detroit', 'dearborn', 'livonia'],
  },
  {
  ...
  },
  {
    ...
    citySlugs: ['minneapolis', 'bloomington', 'brooklyn-park', 'plymouth', 'maple-grove'],
  },
  {
  ...
  },
  {
    ...
    citySlugs: ['atlanta', 'sandy-springs', 'roswell', 'johns-creek'],
  },
  {
  ...
  return getCachedData(
    ...
    async () => {
      try {
        ...
        // Supabase embedded join on a FK (belongsTo) always returns a single object at runtime.
        // The generated TS types may incorrectly type it as an array; we use a type assertion here.
        const stateData = data.state as unknown as {
          name: string
          abbreviation: string
          slug: string
        } | null
...
interface PageProps {
  params: Promise<{ county: string; specialty: string }>
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  let specialtyName = ''
  try {
    const svc = await getSpecialtyBySlug(slug)
  ...
  return {
    ...
    openGraph: {
      ...
      type: 'website',
      locale: 'en_US',
      images: [{ url: getServiceImage(slug).src, width: 1200, height: 630, alt: title }],
    },
    twitter: {
    ...
    <>
      {schemas.map((s, i) => (
        <script
          ...
          dangerouslySetInnerHTML={{ __html: safeJsonStringify(s) }}
        />
      ))}
        ...
        <div className="mx-auto max-w-7xl px-4 py-3">
          <Breadcrumb
            items={[
              ...
              { label: county.name, href: `/counties/${countySlug}` },
              { label: service.name },
            ]}
          />
        </div>
      ...
      <section className="bg-gradient-to-b from-slate-50 to-white">
        <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
          <h1 className="font-heading text-3xl font-bold text-gray-900 sm:text-4xl">{h1}</h1>
          <p className="mt-4 max-w-3xl text-lg text-gray-600">
            {county.name} is located in {county.stateName} with a population of {county.population}.
            The county seat is {county.seat}.{' '}
            {totalCount > 0
              ? `${totalCount} verified ${svcLower} attorneys serve this county.`
              : `Our directory of ${svcLower} attorneys in ${county.name} is growing.`}
          </p>
        </div>
      </section>
      ...
      <section className="border-b bg-white py-10">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h2 className="mb-6 text-2xl font-bold text-gray-900">
            {county.name} Jurisdiction Information
          </h2>
          <div className="grid gap-6 md:grid-cols-3">
            ...
            <div className="rounded-lg border border-gray-200 bg-gray-50 p-5">
              ...
              <p className="mt-2 text-gray-600">
                {county.stateName} ({county.stateCode})
              </p>
            </div>
            <div className="rounded-lg border border-gray-200 bg-gray-50 p-5">
      ...
      <section className="py-10">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h2 className="mb-6 text-2xl font-bold text-gray-900">
            {service.name} Attorneys by City in {county.name}
          </h2>
          {cityCounts.length > 0 ? (
            <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
              {cityCounts.map((cc) => (
                <Link
                  key={cc.slug}
                  href={`/practice-areas/${slug}/${cc.slug}`}
                ...
                >
                  ...
                  <p className="text-sm text-gray-500">
                    {cc.count > 0 ? `${cc.count} attorneys` : 'View directory'}
                  </p>
                </Link>
              ))}
            </div>
            ...
            <p className="text-gray-600">
              City-level data for {county.name} is being compiled. Explore the county directory or
              use the state page.
            </p>
          )}
        </div>
      ...
      <section className="border-t bg-gray-50 py-10">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h2 className="mb-6 text-2xl font-bold text-gray-900">
            Other Practice Areas in {county.name}
          </h2>
          <div className="flex flex-wrap gap-2">
            {otherPAs.map((pa) => (
              <Link
                key={pa.slug}
                href={`/counties/${countySlug}/${pa.slug}`}
                className="rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm transition-colors hover:border-blue-300 hover:text-blue-700"
              >
                {pa.name}
              </Link>
            ))}
      ...
      {state && (
        <section className="border-t bg-white py-10">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <h2 className="mb-4 text-xl font-bold text-gray-900">Statewide Directory</h2>
