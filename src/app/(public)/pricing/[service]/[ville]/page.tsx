...
import { cities, getCityBySlug, getNearbyCities, getStateByCode } from '@/lib/data/usa'
import { getLocationBySlug } from '@/lib/data/commune-data'
...
function getSeasonalTip(zone: string | null, specialtyName: string): string {
  if (zone === 'mediterraneen') {
    return `À noter : le climat méditerranéen favorise les travaux extérieurs quasiment toute l'année. La demande de ${specialtyName.toLowerCase()} peut être plus forte en été avec l'afflux de résidents saisonniers.`
  }
  if (zone === 'montagnard') {
    return `En zone de montagne, les conditions hivernales peuvent limiter certains travaux extérieurs et augmenter les délais d'intervention. Prévoyez vos travaux de ${specialtyName.toLowerCase()} en amont.`
  }
  if (zone === 'continental') {
    return `Avec un climat continental, les écarts de température sont importants. Les travaux de ${specialtyName.toLowerCase()} liés au chauffage et à l'isolation sont particulièrement pertinents.`
  }
  if (zone === 'oceanique' || zone === 'semi-oceanique') {
    return `Le climat océanique implique une humidité fréquente. Les interventions de ${specialtyName.toLowerCase()} liées à l'étanchéité et à la ventilation sont courantes.`
  }
  return `Les conditions climatiques locales peuvent influencer le type et la fréquence des interventions de ${specialtyName.toLowerCase()}.`
}
...
export async function generateMetadata({
  ...
  const villeRegion = getStateByCode(villeData.stateCode)?.region ?? ''
  ...
  const dept = villeData.stateName
  ...
  const canonicalUrl = `${SITE_URL}/pricing/${service}/${villeSlug}`
...
export default async function TarifsServiceVillePage({
  ...
  const commune = await getLocationBySlug(villeSlug)
  ...
  const breadcrumbSchema = getBreadcrumbSchema([
    ...
    { name: `Tarifs ${tradeLower}`, url: `/pricing/${service}` },
    { name: villeData.name, url: `/pricing/${service}/${villeSlug}` },
  ])
  ...
  const serviceSchema = {
    ...
    description: `Service de ${tradeLower} à ${villeData.name} (${villeData.stateName}). Tarifs 2026 : ${minPrice} à ${maxPrice} ${trade.priceRange.unit}.`,
  ...
  const pricingSchema = getServicePricingSchema({
    ...
    description: `Tarifs ${trade.name} à ${villeData.name} : ${minPrice}-${maxPrice} ${trade.priceRange.unit}. Prix ajustés pour la région ${villeRegion}.`,
    lowPrice: minPrice,
    highPrice: maxPrice,
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <SearchRecorder
        ...
        href={`/pricing/${service}/${villeSlug}`}
      />
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-28 md:pt-14 md:pb-36">
          <Breadcrumb
            items={[
              ...
              { label: `Tarifs ${tradeLower}`, href: `/pricing/${service}` },
          ...
          <div className="text-center">
            ...
            <p className="text-xl text-slate-400 max-w-3xl mx-auto mb-4">
              Prix {tradeLower} {'à'} {villeData.name} ({villeData.stateName}) :
            ...
            <div className="flex flex-wrap justify-center gap-3 mt-8">
              ...
              <div className="flex items-center gap-2 bg-white/10 backdrop-blur px-4 py-2 rounded-full border border-white/10 text-sm">
                ...
                <span>{villeData.name} ({villeData.stateCode})</span>
              </div>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            <DemandIndicator specialtySlug={service} cityName={villeData.name} />
          </div>
          ...
          />
          <div className="mt-8">
            <CallbackRequest specialtySlug={service} cityName={villeData.name} />
          </div>
        </div>
      </section>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
        <SpeakableAnswerBox
          answer={`${trade.name} à ${villeData.name} : ${minPrice}–${maxPrice} ${trade.priceRange.unit} (prix ajusté région ${villeRegion}). Prestations courantes : ${trade.commonTasks.slice(0, 3).map(t => t.split(':')[0].trim()).join(', ')}. ${commune?.nb_entreprises_artisanales ? `${commune.nb_entreprises_artisanales} entreprises artisanales dans la commune.` : ''}`}
        />
      </div>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
          ...
          <p className="text-gray-700 leading-relaxed">
            ...
            {multiplier >= 1.2 ? ` Les tarifs en ${villeRegion} sont généralement 20 à 25 % supérieurs à la moyenne nationale.` : ''}
          </p>
      ...
      <section className="py-20 bg-blue-600">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <Link
              href={`/practice-areas/${service}/${villeSlug}`}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
            {otherTrades.map((slug) => {
              ...
              const m = getRegionalMultiplier(villeRegion)
              return (
                <Link
                  key={slug}
                  href={`/pricing/${slug}/${villeSlug}`}
                  ...
                  return (
                    <div key={slug} className="bg-gray-50 rounded-xl border border-gray-200 p-4 space-y-2.5">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        <Link
                          href={`/practice-areas/${slug}/${villeSlug}`}
      ...
      <section className="py-8 border-t">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap gap-3">
            <Link href={`/quotes/${service}/${villeSlug}`} className="px-4 py-2 bg-amber-50 text-amber-800 rounded-lg text-sm font-medium border border-amber-100 hover:border-amber-200 transition-colors">
              Devis {tradeLower} {'à'} {villeData.name}
            </Link>
            <Link href={`/reviews/${service}/${villeSlug}`} className="px-4 py-2 bg-blue-50 text-blue-800 rounded-lg text-sm font-medium border border-blue-100 hover:border-blue-200 transition-colors">
              Avis {tradeLower} {'à'} {villeData.name}
            </Link>
            <Link href={`/emergency/${service}/${villeSlug}`} className="px-4 py-2 bg-red-50 text-red-800 rounded-lg text-sm font-medium border border-red-100 hover:border-red-200 transition-colors">
              Urgence {tradeLower} {'à'} {villeData.name}
            </Link>
            <Link href={`/practice-areas/${service}/${villeSlug}`} className="px-4 py-2 bg-gray-50 text-gray-800 rounded-lg text-sm font-medium border border-gray-200 hover:border-gray-300 transition-colors">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div>
              ...
              <div className="space-y-2">
                <Link href={`/pricing/${service}`} className="block text-sm text-gray-600 hover:text-blue-600 py-1">
                  Tarifs {tradeLower} en France
                </Link>
                <Link href={`/practice-areas/${service}/${villeSlug}`} className="block text-sm text-gray-600 hover:text-blue-600 py-1">
                  {trade.name} {'à'} {villeData.name}
                </Link>
                ...
                <Link href={`/quotes/${service}/${villeSlug}`} className="block text-sm text-gray-600 hover:text-blue-600 py-1">
                  Devis {tradeLower} {'à'} {villeData.name}
                </Link>
                <Link href={`/reviews/${service}/${villeSlug}`} className="block text-sm text-gray-600 hover:text-blue-600 py-1">
                  Avis {tradeLower} {'à'} {villeData.name}
                </Link>
                <Link href={`/emergency/${service}/${villeSlug}`} className="block text-sm text-gray-600 hover:text-blue-600 py-1">
            ...
            <div>
              ...
              <div className="space-y-2">
                <Link href={`/cities/${villeSlug}`} className="block text-sm text-gray-600 hover:text-blue-600 py-1">
                ...
                {otherTrades.slice(0, 5).map((slug) => {
                  ...
                  if (!t) return null
                  return (
                    <Link key={slug} href={`/pricing/${slug}/${villeSlug}`} className="block text-sm text-gray-600 hover:text-blue-600 py-1">
            ...
            <div>
              ...
              <div className="space-y-2">
                ...
                <Link href="/how-it-works" className="block text-sm text-gray-600 hover:text-blue-600 py-1">Comment {'ç'}a marche</Link>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-slate-50 rounded-2xl border border-slate-200 p-6">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              Les prix affich{'é'}s pour {villeData.name} sont des fourchettes indicatives ajust{'é'}es en fonction des donn{'é'}es r{'é'}gionales ({villeRegion}). Ils varient selon la complexit{'é'} du chantier, les mat{'é'}riaux et l&apos;urgence. Seul un devis personnalis{'é'} fait foi. {SITE_NAME} est un annuaire ind{'é'}pendant.
            </p>
          </div>
        </div>
      </section>
      ...
      <StickyMobileCTA specialtySlug={service} citySlug={villeSlug} />

      <EstimationWidget context={{
        metier: trade.name,
        metierSlug: service,
        ville: villeData.name,
        departement: villeData.stateCode,
        pageUrl: `/pricing/${service}/${villeSlug}`,
      }} />

      <ExitIntentPopup
        sessionKey="sa:exit-tarifs-ville"
        description="Obtenez le prix exact pour votre projet — comparez jusqu'à 3 devis gratuits."
        ctaHref={`/quotes/${service}/${villeSlug}`}
      />

      <MicroConversions pageType="tarifs-ville" specialtySlug={service} cityName={villeData.name} />
      <FAQTracker pageType="tarifs-ville" specialtySlug={service} />

      <ProactiveChatPrompt specialtySlug={service} citySlug={villeSlug} />
    </div>
  )
}
