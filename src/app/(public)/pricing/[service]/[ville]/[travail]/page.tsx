...
import { getCityBySlug, getNearbyCities, getStateByCode } from '@/lib/data/usa'
...
export async function generateMetadata({
  ...
  const canonicalUrl = `${SITE_URL}/pricing/${service}/${villeSlug}/${travail}`
...
export default async function TarifsServiceTravailVillePage({
  ...
  let commune = null
  try {
    commune = await getLocationBySlug(villeSlug)
  } catch {
  ...
  const breadcrumbSchema = getBreadcrumbSchema([
    ...
    { name: currentTask.name, url: `/pricing/${service}/${villeSlug}/${travail}` },
  ])
  ...
  const pricingSchema = priceRange
    ? getServicePricingSchema({
        specialtyName: `${currentTask.name} - ${trade.name}`,
        specialtySlug: service,
        description: `Prix ${currentTask.name.toLowerCase()} à ${villeData.name} : ${priceRange.low}-${priceRange.high} €. Tarifs ajustés pour la région ${villeRegion}.`,
        ...
        url: `${SITE_URL}/pricing/${service}/${villeSlug}/${travail}`,
      })
    : null
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-28 md:pt-14 md:pb-36">
          <Breadcrumb
            items={[
              ...
              { label: currentTask.name, href: `/pricing/${service}/${villeSlug}` },
                    ...
                    >
                      <td className={`px-5 py-4 text-sm border-t border-gray-100 ${isHighlighted ? 'text-blue-900' : 'text-gray-800'}`}>
                        {isHighlighted ? (
                          ...
                          <Link
                            href={`/pricing/${service}/${villeSlug}/${tasks[i].slug}`}
      ...
      {relatedCities.length > 0 && (
        <section className="py-16 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-6">
              Prix {currentTask.name.toLowerCase()} dans d&apos;autres cities
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 max-w-3xl">
              {relatedCities.map((v) => {
                const m = getRegionalMultiplier(getStateByCode(v.stateCode)?.region ?? '')
                const range = extractPriceRange(currentTask.priceText, m)
                return (
                  <Link
                    key={v.slug}
                    href={`/pricing/${service}/${v.slug}/${travail}`}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-slate-50 rounded-2xl border border-slate-200 p-6">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              Les prix affich{'é'}s pour {currentTask.name.toLowerCase()} {'à'} {villeData.name} sont des fourchettes indicatives ajust{'é'}es en fonction des donn{'é'}es r{'é'}gionales ({villeRegion}). Ils varient selon la complexit{'é'} du chantier, les mat{'é'}riaux et l&apos;urgence. Seul un devis personnalis{'é'} fait foi. {SITE_NAME} est un annuaire ind{'é'}pendant.
            </p>
          </div>
        </div>
      </section>
