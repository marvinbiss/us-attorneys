...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const seoPairs = hasProviders
    ? [
        {
          title: `${specialtyName} Attorney Cost in ${locationName}`,
          h1: `How Much Does a ${specialtyName} Lawyer Cost in ${locationName}?`,
        },
        {
          title: `${specialtyName} Lawyer Fees — ${locationName} ${currentYear}`,
          h1: `${specialtyName} Attorney Fees in ${locationName}${stateLabel} (${currentYear})`,
        },
        {
          title: `${specialtyName} Cost ${locationName} — Fee Guide`,
          h1: `${specialtyName} Lawyer Cost in ${locationName} — Complete Fee Guide`,
        },
        {
          title: `How Much for ${specialtyName} in ${locationName}?`,
          h1: `${specialtyName} Attorney Cost in ${locationName}${stateLabel}`,
        },
        {
          title: `${specialtyName} Fees ${locationName}${stateLabel}`,
          h1: `${currentYear} ${specialtyName} Attorney Fee Guide — ${locationName}`,
        },
      ]
    : [
        {
        ...
        {
          title: `${specialtyName} Lawyer Fees — ${locationName}`,
          h1: `${specialtyName} Attorney Fees in ${locationName}${stateLabel}`,
        },
        {
          title: `${specialtyName} Cost ${locationName} — Guide`,
          h1: `${specialtyName} Lawyer Cost in ${locationName} — Fee Guide`,
        },
        {
          title: `How Much for ${specialtyName} in ${locationName}?`,
          h1: `${specialtyName} Attorney Cost in ${locationName}${stateLabel}`,
        ...
        {
          title: `${specialtyName} Fees ${locationName}${stateLabel}`,
          h1: `${specialtyName} Attorney Fee Guide — ${locationName}`,
        },
      ]
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6">
      ...
      <p className="mb-6 text-gray-600">
        {service.name} attorneys typically offer several payment options. Understanding these
        structures helps you choose the right arrangement for your case and budget.
      </p>
        ...
        {structures.map((structure) => {
          ...
          return (
            ...
            >
              ...
              <p className="mb-3 text-sm text-gray-600">{structure.description}</p>
              {isRelevant && (
                <div className="text-sm font-medium text-gray-900">
                  {structure.type === 'hourly' && (
                    <span>
                      ${fees.hourly.low}–${fees.hourly.high}/hr in {location.name}
                    </span>
                  )}
                  {structure.type === 'flat_fee' && fees.flatFee && (
                    <span>
                      ${fees.flatFee.low.toLocaleString()}–${fees.flatFee.high.toLocaleString()}
                    </span>
                  )}
                  {structure.type === 'contingency' && fees.contingency && (
                    <span>
                      {fees.contingency.low}%–{fees.contingency.high}% of recovery
                    </span>
                  )}
                  {structure.type === 'retainer' && fees.retainer && (
                    <span>
                      ${fees.retainer.low.toLocaleString()}–${fees.retainer.high.toLocaleString()}{' '}
                      initial deposit
                    </span>
                  )}
                </div>
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6">
        ...
        <table className="w-full border-collapse overflow-hidden rounded-xl">
          <thead>
            <tr className="bg-gray-800 text-white">
              <th className="p-4 text-left font-semibold">Cost Level</th>
              <th className="p-4 text-left font-semibold">Hourly Rate</th>
              {fees.flatFee && <th className="p-4 text-left font-semibold">Flat Fee</th>}
              {fees.contingency && <th className="p-4 text-left font-semibold">Contingency</th>}
              <th className="p-4 text-left font-semibold">Typical Profile</th>
            </tr>
          </thead>
          <tbody>
            <tr className="border-b border-green-100 bg-green-50">
              ...
              {fees.flatFee && (
                <td className="p-4 text-gray-900">${fees.flatFee.low.toLocaleString()}</td>
              )}
            ...
            <tr className="border-b border-yellow-100 bg-yellow-50">
              ...
              <td className="p-4 text-sm text-gray-600">
                Small/mid-size firm, 5–15 years experience
              </td>
            </tr>
            <tr className="bg-red-50">
              ...
              {fees.flatFee && (
                <td className="p-4 text-gray-900">${fees.flatFee.high.toLocaleString()}</td>
              )}
              {fees.contingency && <td className="p-4 text-gray-900">{fees.contingency.high}%</td>}
              <td className="p-4 text-sm text-gray-600">
                Large firm / specialist, 15+ years experience
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <p className="mt-3 text-xs text-gray-500">
        * Rates adjusted for {fees.adjustment.label.toLowerCase()} (
        {location.department_name || location.name}). Actual fees depend on case specifics.
      </p>
    </div>
  )
}
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6">
      ...
      <div className="mb-4 rounded-xl border border-gray-200 bg-white p-6">
        <h3 className="mb-3 font-semibold text-gray-900">{service.name}-Specific Cost Factors</h3>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {generalFactors.map((item) => (
          <div key={item.factor} className="rounded-xl border border-gray-200 bg-white p-4">
            <div className="mb-2 flex items-center gap-2">
              <h4 className="text-sm font-semibold text-gray-900">{item.factor}</h4>
              <span
...
export default async function CostGuidePage({ params }: PageProps) {
  ...
  let cmsPage = null
  try {
    cmsPage = await getPageContent(`cost-${specialtySlug}-${locationSlug}`, 'location', {
      specialtySlug,
      locationSlug,
    })
  } catch (err: unknown) {
    logger.error('[CMS] Error fetching page content for', {
      slug: `cost-${specialtySlug}-${locationSlug}`,
      error: err,
    })
  }
  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-7xl px-4 py-4">
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-6">
          <h2 className="mb-3 text-lg font-semibold text-amber-900">
            {service.name} Attorney Fee Overview — {location.name}
            {stateLabel}
          </h2>
          <div className="grid grid-cols-1 gap-4 text-sm sm:grid-cols-3">
            <div className="rounded-lg border border-amber-100 bg-white p-4">
              <p className="mb-1 font-medium text-amber-600">Hourly Rate</p>
              <p className="text-2xl font-bold text-gray-900">
                ${minPrice}–${maxPrice}
              </p>
              <p className="mt-1 text-xs text-gray-500">{trade?.priceRange.unit || 'per hour'}</p>
            </div>
            <div className="rounded-lg border border-amber-100 bg-white p-4">
              <p className="mb-1 font-medium text-amber-600">Contingency Fee</p>
              <p className="text-2xl font-bold text-gray-900">25–40%</p>
              <p className="mt-1 text-xs text-gray-500">of settlement amount</p>
            </div>
            <div className="rounded-lg border border-amber-100 bg-white p-4">
              <p className="mb-1 font-medium text-amber-600">Free Consultation</p>
              <p className="text-2xl font-bold text-green-700">$0</p>
              <p className="mt-1 text-xs text-gray-500">initial case review</p>
            </div>
          </div>
          <p className="mt-3 text-xs text-amber-700">
            * Fees based on {currentYear} {location.department_name || location.name} market data.
            Actual costs vary by case complexity and attorney experience.
          </p>
        </div>
      </div>
      ...
      <CostFactorsSection service={service} location={location} specialtySlug={specialtySlug} />
