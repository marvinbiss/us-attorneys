...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const seoHash = Math.abs(hashCode(`hire-seo-${specialtySlug}-${locationSlug}`))
  ...
  const descHash = Math.abs(hashCode(`hire-desc-${specialtySlug}-${locationSlug}`))
  const deptLabel = departmentName || departmentCode
  const descTemplates = hasProviders
    ? [
        `Hire a top ${svcLower} attorney in ${locationName}${deptLabel ? ` (${deptLabel})` : ''}. ${attorneyCount} verified lawyers. Free consultation, no obligation.`,
        `Ready to hire a ${svcLower} lawyer in ${locationName}? Compare ${attorneyCount} bar-verified attorneys. Book a free consultation today.`,
        `Hire the right ${svcLower} attorney in ${locationName}. ${attorneyCount} verified professionals. Reviews, fees, and free initial consultation.`,
        `${locationName}${departmentCode ? ` (${departmentCode})` : ''}: hire from ${attorneyCount} verified ${svcLower} attorneys. Free consultation available.`,
        `Looking to hire a ${svcLower} attorney in ${locationName}? ${attorneyCount} qualified lawyers. Compare and book a free consultation.`,
      ]
    : [
        `Hire a qualified ${svcLower} attorney in ${locationName}${deptLabel ? ` (${deptLabel})` : ''}. Bar-verified lawyers. Free consultation available.`,
        `Ready to hire a ${svcLower} lawyer in ${locationName}? Find verified attorneys. Free consultation, no obligation.`,
        `Hire the right ${svcLower} attorney in ${locationName}. Directory of verified professionals. Free initial consultation.`,
        `${locationName}${deptLabel ? ` (${deptLabel})` : ''}: find and hire a trusted ${svcLower} attorney. Free consultation.`,
        `Hire a ${svcLower} attorney in ${locationName}. Bar-verified professionals. Free and immediate consultation.`,
      ]
  ...
  return {
    ...
    alternates: {
      canonical: `${SITE_URL}/hire/${specialtySlug}/${locationSlug}`,
      languages: getAlternateLanguages(`/hire/${specialtySlug}/${locationSlug}`),
    },
  }
}

// JSON-LD structured data — LegalService with offers (free consultation)
function generateJsonLd(
  ...
  const legalServiceSchema: Record<string, unknown> = {
    ...
    '@type': 'LegalService',
    name: `Hire a ${service.name} Attorney in ${location.name}`,
    description: `Hire a verified ${svcLower} attorney in ${location.name}. Free initial consultation available.`,
    url: `${SITE_URL}/hire/${specialtySlug}/${locationSlug}`,
    ...
    offers: {
      ...
      price: '0',
      priceCurrency: 'USD',
      description: `Free initial consultation with a ${svcLower} attorney in ${location.name}`,
  ...
  const collectionPageSchema: Record<string, unknown> = {
    ...
    '@type': 'CollectionPage',
    name: `Hire a ${service.name} Attorney in ${location.name}`,
    description: `Find and hire a verified ${svcLower} attorney in ${location.name}. Compare profiles and book a free consultation.`,
    url: `${SITE_URL}/hire/${specialtySlug}/${locationSlug}`,
    isPartOf: {
      '@type': 'WebSite',
      name: 'US Attorneys',
  ...
  const breadcrumbSchema = getBreadcrumbSchema([
    ...
    { name: service.name, url: `/hire/${specialtySlug}` },
    { name: location.name, url: `/hire/${specialtySlug}/${locationSlug}` },
  ])

  return [legalServiceSchema, collectionPageSchema, breadcrumbSchema]
}

export default async function HireAttorneyPage({ params }: PageProps) {
  const { specialty: specialtySlug, location: locationSlug } = await params
  ...
  return (
    <>
      ...
      <SearchRecorder
        type="service-city"
        label={`Hire ${service.name} Attorney in ${location.name}`}
        href={`/hire/${specialtySlug}/${locationSlug}`}
      />
