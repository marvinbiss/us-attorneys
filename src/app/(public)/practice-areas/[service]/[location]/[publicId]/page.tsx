...
interface ProviderRecord {
  ...
  opening_hours?: Record<string, { open: boolean; start: string; end: string }> | null
...
async function getSimilarAttorneys(attorneyId: string, specialty: string, postalCode?: string) {
  try {
    ...
    let query = supabase
      .from('attorneys')
      .select(
        'id, stable_id, slug, name, primary_specialty_id, rating_average, review_count, address_city, is_verified, phone, specialty:specialties!primary_specialty_id(slug,name)'
      )
      .eq('is_active', true)
      .neq('id', attorneyId)
...
async function getAttorneySpecialtiesList(
  attorneyId: string
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (quartierMatch) {
    ...
    let attorneyCount = 0
    try {
      attorneyCount = await getAttorneyCountByServiceAndLocation(specialtySlug, locationSlug)
    } catch {
      /* best-effort */
    }
    ...
    return {
      ...
      openGraph: {
        ...
        type: 'website',
        locale: 'en_US',
        url: `${SITE_URL}/practice-areas/${specialtySlug}/${locationSlug}/${publicId}`,
      ...
      alternates: {
        canonical: `${SITE_URL}/practice-areas/${specialtySlug}/${locationSlug}/${publicId}`,
      },
    }
  }
  ...
  try {
    ...
    if (!rawProvider)
      return { title: 'Attorney Not Found', robots: { index: false, follow: false } }
    ...
    if (provider.review_count && provider.review_count > 0) {
      descParts.push(
        `${provider.review_count} reviews${provider.rating_average ? ` (${Number(provider.rating_average).toFixed(1)}/5)` : ''}`
      )
    }
    ...
    const description =
      rawDesc.length > 155 ? rawDesc.slice(0, 154).replace(/\s+\S*$/, '') + '…' : rawDesc
...
export default async function AttorneyPage({ params }: PageProps) {
  ...
  let enrichment: AttorneyEnrichmentData = {
    education: [],
    awards: [],
    publications: [],
    disciplinary: [],
    ...
    <>
          ...
          <Breadcrumbs
            items={[
              ...
              ...(attorney.department_code
                ? (() => {
                    const stateData = getStateByCode(attorney.department_code as string)
                    return stateData
                      ? [
                          {
                            label: stateData.name,
                            href: `/states/${stateData.slug}`,
                            semanticType: 'AdministrativeArea' as const,
                          },
                        ]
                      : []
                  })()
      ...
      <section className="border-t bg-white py-6">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <Link
            href={`/practice-areas/${specialtySlug}/${locationSlug}`}
            className="inline-flex items-center gap-2 rounded-xl border border-gray-200 bg-gray-50 px-5 py-3 text-sm font-medium text-gray-700 transition-all hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700"
          >
            <svg
              className="h-4 w-4"
              fill="none"
