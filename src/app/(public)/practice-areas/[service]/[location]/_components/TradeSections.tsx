      ...
      <section className="py-12 border-t">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-xl font-bold text-gray-900 mb-1 border-l-4 border-amber-500 pl-4">
            Why Hire a Professional {service.name.toLowerCase()} in {location.name}?
          </h2>
          <p className="text-gray-600 text-sm mt-2 pl-[calc(1rem+4px)]">
            Essential criteria for finding a reliable and competent {service.name.toLowerCase()} in {location.name}{location.department_name ? `, ${location.department_name}` : ''}.
          </p>
          <div className="mt-6 space-y-4">
            {trade.certifications && trade.certifications.length > 0 && (
              <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4">
                <h3 className="font-semibold text-emerald-900 mb-2">Certifications and Guarantees in {location.name}</h3>
                <p className="text-sm text-emerald-800 mb-3">
                  Key qualifications to look for in a {service.name.toLowerCase()} in {location.name} ({location.department_code}):
                </p>
            ...
            {selectedTips.length > 0 && (
              <div className="bg-clay-50 border border-clay-100 rounded-xl p-4">
                <h3 className="font-semibold text-stone-800 mb-2">Expert Tips for {location.name}</h3>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
            ...
            <p className="text-gray-600 mb-6 text-sm pl-[calc(1rem+4px)]">
              Average hourly fee in {location.name}: <strong className="text-gray-900">{Math.round(trade.priceRange.min * pricingMultiplier)}–{Math.round(trade.priceRange.max * pricingMultiplier)} {trade.priceRange.unit}</strong>.
              {pricingMultiplier !== 1.0 && ` Fees adjusted for the ${location.name} area.`}
              {pricingMultiplier === 1.0 && ` Prices in ${location.name} may vary depending on case complexity and the professional chosen.`}
            </p>
            <PriceTable
            ...
            {trade.emergencyInfo && (
              <div className="mt-6 p-4 bg-red-50 border border-red-100 rounded-xl">
                <p className="text-sm text-red-800">
                  <strong>Emergency {service.name.toLowerCase()} in {location.name}:</strong>{' '}
                  {trade.averageResponseTime}
                </p>
              </div>
            )}
            <p className="text-xs text-gray-400 mt-4">
              The fees displayed are indicative and based on market averages{location.region_name ? ` in ${location.region_name}` : ''} for a {service.name.toLowerCase()} in {location.name}.{' '}
              Request a personalized consultation to get an accurate price for your case.
            </p>
            <Link
            ...
            >
              View all {service.name.toLowerCase()} fees nationwide
