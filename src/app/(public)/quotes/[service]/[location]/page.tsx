...
import { getLocationBySlug, formatNumber, formatEuro } from '@/lib/data/commune-data'
...
export async function generateMetadata({
  ...
  const villeData = getCityBySlug(location)
  ...
  const canonicalUrl = `${SITE_URL}/quotes/${service}/${location}`
...
export default async function DevisServiceLocationPage({
  ...
  const commune = await getLocationBySlug(location)
  ...
  const breadcrumbSchema = getBreadcrumbSchema([
    ...
    { name: villeData.name, url: `/quotes/${service}/${location}` },
  ])
  ...
  const serviceSchema = {
    ...
    description: `Demandez un devis gratuit pour ${tradeLower} à ${villeData.name} (${villeData.stateName}). Prix : ${minPrice}–${maxPrice} ${trade.priceRange.unit}.`,
    provider: {
      '@type': 'Organization',
      name: SITE_NAME,
      url: SITE_URL,
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-28 md:pt-14 md:pb-36">
          ...
          <div className="text-center">
            ...
            <p className="text-xl text-slate-400 max-w-3xl mx-auto mb-4">
              Comparez jusqu&apos;à 3 devis de {tradeLower}s à {villeData.name} ({villeData.stateName}).
      ...
      <section id="formulaire" className="relative -mt-16 z-10 px-4 pb-20">
        <DevisForm
          ...
          prefilledCityPostal={villeData.zipCode}
        />
      </section>
      ...
      <section className="py-20 bg-blue-600">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            ...
            <Link
              href={`/practice-areas/${service}/${location}`}
      ...
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-6">
            Devis {tradeLower} dans d&apos;autres cities
          </h2>
                  ...
                  return (
                    <div key={slug} className="bg-white rounded-xl border border-gray-200 p-4 space-y-2.5">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        <Link
                          href={`/practice-areas/${slug}/${location}`}
      ...
      <section className="py-8 border-t">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap gap-3">
            <Link href={`/reviews/${service}/${location}`} className="px-4 py-2 bg-blue-50 text-blue-800 rounded-lg text-sm font-medium border border-blue-100 hover:border-blue-200 transition-colors">
              Avis {tradeLower} à {villeData.name}
            </Link>
            <Link href={`/pricing/${service}/${location}`} className="px-4 py-2 bg-emerald-50 text-emerald-800 rounded-lg text-sm font-medium border border-emerald-100 hover:border-emerald-200 transition-colors">
              Tarifs {tradeLower} à {villeData.name}
            </Link>
            <Link href={`/emergency/${service}/${location}`} className="px-4 py-2 bg-red-50 text-red-800 rounded-lg text-sm font-medium border border-red-100 hover:border-red-200 transition-colors">
              Urgence {tradeLower} à {villeData.name}
            </Link>
            <Link href={`/practice-areas/${service}/${location}`} className="px-4 py-2 bg-gray-50 text-gray-800 rounded-lg text-sm font-medium border border-gray-200 hover:border-gray-300 transition-colors">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div>
              ...
              <div className="space-y-2">
                ...
                <Link href={`/pricing/${service}/${location}`} className="block text-sm text-gray-600 hover:text-blue-600 py-1">
                  Tarifs {tradeLower} à {villeData.name}
                </Link>
                <Link href={`/practice-areas/${service}/${location}`} className="block text-sm text-gray-600 hover:text-blue-600 py-1">
                  {trade.name} à {villeData.name}
                </Link>
                ...
                <Link href={`/reviews/${service}/${location}`} className="block text-sm text-gray-600 hover:text-blue-600 py-1">
                  Avis {tradeLower} à {villeData.name}
                </Link>
                <Link href={`/emergency/${service}/${location}`} className="block text-sm text-gray-600 hover:text-blue-600 py-1">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-slate-50 rounded-2xl border border-slate-200 p-6">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              Les prix affichés pour {villeData.name} sont des fourchettes indicatives ajustées en fonction des données régionales ({villeRegion}). Ils varient selon la complexité du chantier, les matériaux et l&apos;urgence. Seul un devis personnalisé fait foi. {SITE_NAME} est un annuaire indépendant.
            </p>
          </div>
        </div>
      </section>
