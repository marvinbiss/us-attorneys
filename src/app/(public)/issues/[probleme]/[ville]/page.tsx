...
import { getLocationBySlug, formatNumber, type LocationData } from '@/lib/data/commune-data'
...
export async function generateMetadata({
  ...
  const metaRegion = getStateByCode(villeData.stateCode)?.region ?? ''
  const multiplier = getRegionalMultiplier(metaRegion)
  ...
  return {
    title,
    description,
    alternates: { canonical: `${SITE_URL}/issues/${probleme}/${ville}` },
    robots: { index: true, follow: true },
    openGraph: {
      locale: 'fr_FR',
      title,
      description,
      url: `${SITE_URL}/issues/${probleme}/${ville}`,
      type: 'website',
...
export default async function ProblemeVillePage({
  ...
  const commune = await getLocationBySlug(ville)
  ...
  const breadcrumbSchema = getBreadcrumbSchema([
    { name: 'Accueil', url: '/' },
    { name: 'Problèmes', url: '/issues' },
    { name: problem.name, url: `/issues/${probleme}` },
    { name: villeData.name, url: `/issues/${probleme}/${ville}` },
  ])
  ...
  const serviceSchema = {
    ...
    description: `Diagnostic et résolution de ${problem.name.toLowerCase()} à ${villeData.name} (${villeData.stateName}). Coût : ${minPrice} à ${maxPrice} €.`,
    provider: {
      '@type': 'Organization',
      name: SITE_NAME,
      url: SITE_URL,
      ...
      <div className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <Breadcrumb items={[
            ...
            { label: problem.name, href: `/issues/${probleme}` },
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap gap-3 mb-8">
            ...
            <div className="flex items-center gap-2 bg-white/10 backdrop-blur-sm rounded-full px-4 py-2 border border-white/10">
              ...
              <span className="text-sm">{villeData.name} ({villeData.stateCode})</span>
            </div>
            <div className="flex items-center gap-2 bg-white/10 backdrop-blur-sm rounded-full px-4 py-2 border border-white/10">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <Link
              href={`/quotes/${problem.primaryService}/${ville}`}
            ...
            <Link
              href={`/emergency/${problem.primaryService}/${ville}`}
            ...
            <Link
              href={`/practice-areas/${problem.primaryService}/${ville}`}
              className="inline-flex items-center justify-center gap-3 bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/20 text-white px-8 py-4 rounded-xl font-semibold text-lg transition-all"
            >
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-2 gap-6">
            ...
            <LocalFactorCard
              ...
              description={`${villeData.name} est une commune de ${villeData.stateName} (${villeRegion}). La densité de population influence les délais d'intervention des artisans.`}
            />
          </div>
        </div>
      </section>
        ...
        <section className="py-16 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-6">
              {problem.name} dans d&apos;autres cities
            </h2>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 max-w-3xl">
              {nearbyCities.map((v) => (
                <Link
                  key={v.slug}
                  href={`/issues/${probleme}/${v.slug}`}
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div>
              ...
              <div className="space-y-2">
                <Link href={`/issues/${probleme}`} className="block text-sm text-gray-600 hover:text-amber-600 py-1">
                  {problem.name} en France
                </Link>
                <Link href={`/quotes/${problem.primaryService}/${ville}`} className="block text-sm text-gray-600 hover:text-amber-600 py-1">
                  Devis {tradeName.toLowerCase()} à {villeData.name}
                </Link>
                <Link href={`/practice-areas/${problem.primaryService}/${ville}`} className="block text-sm text-gray-600 hover:text-amber-600 py-1">
                  {tradeName} à {villeData.name}
                </Link>
                <Link href={`/emergency/${problem.primaryService}/${ville}`} className="block text-sm text-gray-600 hover:text-amber-600 py-1">
                  {tradeName} urgence à {villeData.name}
                </Link>
                <Link href={`/pricing/${problem.primaryService}/${ville}`} className="block text-sm text-gray-600 hover:text-amber-600 py-1">
                  Tarifs {tradeName.toLowerCase()} à {villeData.name}
                </Link>
                <Link href={`/reviews/${problem.primaryService}/${ville}`} className="block text-sm text-gray-600 hover:text-amber-600 py-1">
            ...
            <div>
              ...
              <div className="space-y-2">
                <Link href={`/cities/${ville}`} className="block text-sm text-gray-600 hover:text-amber-600 py-1">
                  Artisans à {villeData.name}
                </Link>
                {relatedProblems.slice(0, 3).map((rp) => (
                  <Link key={rp.slug} href={`/issues/${rp.slug}/${ville}`} className="block text-sm text-gray-600 hover:text-amber-600 py-1">
                    {rp.name} à {villeData.name}
                  </Link>
                ))}
              </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-slate-50 rounded-2xl border border-slate-200 p-6">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              Les prix affichés pour {villeData.name} sont des fourchettes indicatives ajustées en fonction des données régionales ({villeRegion}). Ils varient selon la complexité du problème et l&apos;urgence. Seul un devis personnalisé fait foi. {SITE_NAME} est un annuaire indépendant — nous mettons en relation mais ne réalisons pas les interventions.
            </p>
          </div>
        </div>
      </section>
