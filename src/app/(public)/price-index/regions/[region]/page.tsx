...
import { BAROMETRE_REGIONS, getBarometreRegionBySlug, getBarometreMetierBySlug } from '@/lib/price-index/constants'
import { getStatsByRegion } from '@/lib/price-index/queries'
import { regionalIndices } from '@/lib/data/price-index'
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const description = `Baromètre des artisans en ${region.name} : top métiers, ${region.states.length} départements, notes moyennes et taux de vérification. Données actualisées ${SITE_NAME}.`
  const canonicalUrl = `${SITE_URL}/price-index/regions/${regionSlug}`
...
export default async function BarometreRegionPage({ params }: PageProps) {
  ...
  const faqItems = [
    {
      ...
      answer: totalArtisans > 0
        ? `Notre annuaire recense ${totalArtisans.toLocaleString('fr-FR')} artisans actifs en ${region.name}, répartis dans ${region.states.length} départements et couvrant plus de ${stats.length} corps de métier.`
  ...
  const breadcrumbSchema = getBreadcrumbSchema([
    { name: 'Accueil', url: '/' },
    { name: 'Baromètre', url: '/price-index' },
    { name: 'Régions', url: '/price-index/regions' },
    { name: region.name, url: `/price-index/regions/${regionSlug}` },
  ])
    ...
    <>
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
          <Breadcrumb
            items={[
              { label: 'Baromètre', href: '/price-index' },
              { label: 'Régions', href: '/price-index/regions' },
              { label: region.name },
            ]}
          />
        </div>
        ...
        <header className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-12">
          <div className="max-w-3xl">
            ...
            <p className="text-lg text-gray-600">
              Baromètre des artisans en {region.name} : {region.states.length} départements,
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5 text-center">
              ...
              <div className="text-2xl font-bold text-gray-900">{region.states.length}</div>
            ...
            <div className="space-y-3">
              {stats.slice(0, 15).map((row, idx) => {
                ...
                return (
                  <Link
                    key={row.metier_slug}
                    href={`/price-index/pricing/${row.metier_slug}`}
        ...
        <section className="bg-white py-16">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {region.states.map((dept) => (
                <Link
                  key={dept.code}
                  href={`/states/${dept.nom.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]+/g, '-')}`}
