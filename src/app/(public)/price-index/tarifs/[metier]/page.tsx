...
import { BAROMETRE_METIERS, getBarometreMetierBySlug, TOP_VILLES } from '@/lib/price-index/constants'
import { getStatsByMetier, getMetierTopVilles } from '@/lib/price-index/queries'
import type { BarometreStatRow } from '@/lib/price-index/queries'
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonicalUrl = `${SITE_URL}/price-index/pricing/${metierSlug}`
...
export default async function BarometreMetierPage({ params }: PageProps) {
  ...
  const breadcrumbSchema = getBreadcrumbSchema([
    ...
    { name: 'Métiers', url: '/price-index/pricing' },
    { name: metier.label, url: `/price-index/pricing/${metierSlug}` },
  ])
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
          <Breadcrumb
            items={[
              ...
              { label: 'Métiers', href: '/price-index/pricing' },
              { label: metier.label },
            ]}
          />
        </div>
                  ...
                  <tbody>
                    {villeStats.map((row, idx) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-right">
                          <Link
                            href={`/practice-areas/${metierSlug}/${row.ville_slug}`}
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex flex-wrap gap-2">
              {TOP_VILLES.slice(0, 10).map((ville) => (
                <Link
                  key={ville}
                  href={`/practice-areas/${metierSlug}/${ville.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]+/g, '-')}`}
