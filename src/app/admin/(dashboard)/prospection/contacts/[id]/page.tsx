...
export default function ContactDetailPage() {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => null)
        setActionError(data?.error?.message || 'Error updating contact')
        return
      }
      const data = await res.json()
      if (data.success) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            {editing ? (
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-3 text-sm">
                  ...
                  <span className="text-gray-400 text-xs">{new Date(contact.created_at).toLocaleDateString('en-US')}</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-lg border p-6">
            ...
            {messages.length === 0 ? (
              <p className="text-sm text-gray-400 text-center py-6">No messages sent to this contact.</p>
            ) : (
              <div className="space-y-3">
                {messages.map((msg) => (
                  <div key={msg.id} className="border rounded-lg p-3 text-sm">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-full ${
                        ...
                        {msg.status === 'delivered' ? 'Delivered'
                          ...
                          : msg.status === 'bounced' ? 'Bounced'
                          : msg.status === 'opted_out' ? 'Opted out'
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      {msg.sent_at ? new Date(msg.sent_at).toLocaleString('en-US') : 'Not sent'}
                    </p>
                  </div>
                ))}
              </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-lg border p-4">
            <h3 className="text-sm font-medium flex items-center gap-2 mb-3">
              <Shield className="w-4 h-4 text-gray-400" /> GDPR / Consent
            </h3>
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                  ...
                  {contact.consent_status === 'opted_in' ? 'Opted in'
                    : contact.consent_status === 'opted_out' ? 'Opted out'
                    : 'Unknown'}
                </span>
              </div>
              {contact.opted_out_at && (
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-500">Opted out on</span>
                  <span className="text-xs text-gray-400">{new Date(contact.opted_out_at).toLocaleDateString('en-US')}</span>
                </div>
              )}
            </div>
          </div>
          ...
          {/* Lists membership */}
          <div className="bg-white rounded-lg border p-4">
            <h3 className="text-sm font-medium mb-3">Lists</h3>
            {lists.length === 0 ? (
              <p className="text-sm text-gray-400">No lists loaded</p>
            ) : (
